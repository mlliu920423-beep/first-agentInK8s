...
- 收集 golden set：prompt + 期望的 agent 路由 / tool 调用序列
- 写 e2e test：pick a prompt, run HostMA, assert on the events
- 引入 LLM judge 打分（正确性、简洁性、tool usage 合理性）

**先能测什么：** SSE event stream 是完全可观测的，`kubectl logs` + `curl -N /api/chat?q=...` 就能拿到全部事件序列。
...
*本文档最后一次全面对齐代码：2026-07-11（commit `fa9c055`）。若代码有较大重构，请对应更新引用行号。*
