...
sequenceDiagram
  ...
  M->>Loader: 扫 MCP_DIR/*.yaml (dictionary order)
  loop 每份 yaml
...
**为什么先于 MCP：** 内置 tool 是纯 Go 代码，注册"必成功"。MCP loader（步骤 3）在 Phase 1 之后同样是 fail-fast —— 但把稳的先放好，日志里看 Registry 状态更容易分辨"是内置注册出问题还是 MCP 出问题"。

### 步骤 3：MCP Sources（`main.go:73-79`）

**从 Phase 1 起改为声明式 + fail-fast**：`mcp.LoadAll(ctx, MCP_DIR, reg)` 扫 `mcp/*.yaml`，按文件名字典序依次评估 `enabled_if`、分派到对应 driver、注册 tool。**任一被启用的 MCP 启动失败都是硬错（pod crashloop）**，历史上的"warn-and-continue"已经废弃 —— 参见 [§6](#6-mcp-集成) 和 [ADR-005](docs/adr/005-mcp-driver-abstraction.md)。

**`fs.` 前缀魔法命名空间保留**：`ENABLE_FS_MCP` 未设置时 `mcp/filesystem.yaml` 走 `enabled_if=env:ENABLE_FS_MCP=1` 判定为 disabled，Registry 里就没有 `fs.*` —— Specialist 引用 `fs.read_file` 时 `MustResolve` 按可选跳过（`registry.go:69-90`）。这跟 Phase 0 的用户可见行为一致，只是"跳过"的位置从 `filesystem.go` 上移到了 loader。
...
MCP（Model Context Protocol）是 Anthropic 提的一套"LLM ↔ 外部工具"通信协议。**Phase 1（2026-07-17）之后，本项目集成方式从"main.go 里两个硬编码调用"转成"声明式 loader + driver 抽象"**。设计动机与决策见：
...
- **Phase 2**：Registry 加 `Unregister`；Host 用 `atomic.Pointer` 换 specialist；实现"运行时挂/摘 MCP"而不重启 pod
- **Phase 3**：REST API `/api/mcp` CRUD，把 yaml 声明面搬到运行时 API
- **Phase 4**：前端 UI 编辑 MCP 声明，形成完整 workbuddy 控制面
