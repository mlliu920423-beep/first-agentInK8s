...
**当前 commit：** `54be7be`（2026-07-15），Phase 2 编排层变更覆盖至 2026-07-17。
...
flowchart TB
  ...
  subgraph Go[Go 二进制 :8080]
    ...
    subgraph Compose[编排层]
      Sup[agents.Supervisor<br/>atomic.Pointer&lt;MultiAgent&gt;]
    ...
    subgraph Tools[能力层]
      ...
      Time[current_time]
      Weather[weather]
      InProc[mcp.echo / mcp.list_dir<br/>inproc driver]
      FS[fs.* 外部 npx MCP<br/>stdio driver]
    end
...
sequenceDiagram
  ...
  participant Sup as agents.NewSupervisor
  participant Loader as mcp.LoadAll
  participant Drv as Driver (inproc/stdio)
  ...
  M->>Sup: NewSupervisor(ctx, deps)
  Note over Sup: 内含 MCP LoadAll<br/>+ agentcfg.Load<br/>+ BuildSpecialist × N<br/>+ BuildHost（事务化）
  Sup->>Loader: 扫 MCP_DIR/*.yaml (dictionary order)
  loop 每份 yaml
    Loader->>Loader: parseConfig + parseCondition
    alt enabled_if=false
      Loader-->>Sup: skip + log
    else enabled_if=true
      Loader->>Drv: Start(ctx, cfg, reg)
      Drv-->>Reg: 注册 <cfg.name>.<tool>
      Drv-->>Loader: io.Closer (or error → 硬 fail)
    end
  end
  Sup->>Cfg: 读 agents/*.yaml
  Cfg-->>Sup: []AgentConfig
  loop 每个 config
    Sup->>Build: BuildSpecialist(model, cfg, reg)
    Build-->>Sup: *host.Specialist
  end
  Sup->>Host: BuildHost(model, DefaultHostPrompt, specialists)
  Host-->>Sup: *host.MultiAgent
  Note over Sup: atomic.Store(&ptr, hostMA)<br/>owning mcpClosers
  Sup-->>M: *Supervisor
  M->>CB: InstallToolCallbacks（全局，仅一次）
  M->>HTTP: ListenAndServe :8080（Server.Sup = sup）
```
...
sequenceDiagram
  ...
  H->>SP0: sup.Current()（lock-free）
  SP0-->>H: *MultiAgent（请求生命周期内稳定）
...
**Phase 2（2026-07-17）起从 8 步简化为 5 步**：原来 step 3-6（MCP loader / agentcfg / BuildSpecialist / BuildHost）全部塞进 `agents.NewSupervisor` 一次调用，`main.go` 只保留骨架编排。Supervisor 的动机、事务化 rebuild 语义与并发约束见：

- Spec：[`docs/specs/phase-2-registry-mutation-host-swap.md`](docs/specs/phase-2-registry-mutation-host-swap.md)
- ADR：[`docs/adr/006-registry-mutation-host-swap.md`](docs/adr/006-registry-mutation-host-swap.md)

### 步骤 1：Ark ChatModel

**为什么第一步：** 环境变量 `ARK_API_KEY` / `ARK_MODEL_ID` 缺失就 `log.Fatalf` —— **fail fast**。否则可能启动到 Supervisor 装配阶段才发现连不上模型，白干前面的一堆 MCP 子进程。

**共享一份还是每个 agent 一份：** 现在是共享一份，传给所有 specialist 和 host。这是简化的假设 —— 见 [§4](#4-llm-抽象) 讨论。

### 步骤 2：Tool Registry + Builtins

**为什么先于 Supervisor：** 内置 tool 是纯 Go 代码，注册"必成功"。Supervisor 内部的 MCP loader 是 fail-fast —— 把稳的先放好，日志里看 Registry 状态更容易分辨"是内置注册出问题还是 MCP 出问题"。

### 步骤 3：Supervisor（原 step 3-6 合并）

`agents.NewSupervisor(ctx, deps)` 内部按顺序完成：

1. `mcp.LoadAll(ctx, MCP_DIR, reg)` —— 扫 `mcp/*.yaml`，按文件名字典序启动所有 driver，返回 `[]io.Closer`。**Supervisor 是 mcpClosers 的所有者**（不再是 `main.go`），rebuild 时新旧 closers 的 swap 也在这里发生。
2. `agentcfg.Load(AGENTS_DIR)` —— 读 `agents/*.yaml`。
3. `agents.BuildSpecialist(model, cfg, reg)` × N —— 每份 config 一个 `*host.Specialist`。
4. `agents.BuildHost(model, DefaultHostPrompt, specialists)` —— 组装 `*host.MultiAgent`。
5. `atomic.Store(&ptr, hostMA)` —— 把 host 塞进 `atomic.Pointer[host.MultiAgent]`；此后 SSE 层通过 `sup.Current()` 无锁读。

任一子步骤失败：**已启动的 closers 反序 Close，返回硬错误**（pod crashloop）。事务化语义确保"半成品 Supervisor 永远不会被暴露给 HTTP 层"。

**`fs.` 前缀魔法命名空间保留**：`ENABLE_FS_MCP` 未设置时 `mcp/filesystem.yaml` 走 `enabled_if=env:ENABLE_FS_MCP=1` 判定为 disabled，Registry 里就没有 `fs.*`；Specialist 引用 `fs.read_file` 时 `MustResolve` 按可选跳过（`registry.go:69-90`）。

### 步骤 4：全局 Tool 回调

**`httpapi.InstallToolCallbacks()` 只在这里调一次**（`main.go` step 4），通过 `callbacks.AppendGlobalHandlers` 写到 Eino 全局注册表。必须在第一次请求进来前挂好，放在 HTTP listen 之后就有竞态。

**硬约束（Phase 2 caveat）：Supervisor.Rebuild 严禁调 InstallToolCallbacks**。Eino 官方文档明确 `AppendGlobalHandlers` **不幂等且非线程安全**，重复调会产生重复回调、并发调会数据竞争。这也是为什么 rebuild 只 swap host + closers，不重装 callback。风险推理详见 [`docs/research/phase-2-host-swap-risks.md`](docs/research/phase-2-host-swap-risks.md)。

### 步骤 5：HTTP 服务器

三个路由：`/healthz`（k8s probe）、`/api/chat`（SSE 主入口）、`/`（embed 前端 SPA fallback）。

**关键改动**：`httpapi.Server` 结构从 `HostMA *host.MultiAgent` 迁到 `Sup *agents.Supervisor`。`HandleChat` **首行** `hostMA := s.Sup.Current()`，之后请求生命周期内都用这一份引用 —— 请求内绝不重复 `Current()`，避免中途看到"新旧混用"。

### 优雅关闭

`signal.NotifyContext` 捕获 SIGINT/SIGTERM，10s 超时 shutdown HTTP，再调 `sup.Shutdown(ctx)` —— 后者关闭所有 MCP client（外部 MCP 的 stdio 子进程也一并回收）。
...
**目录：** [`internal/mcp/`](internal/mcp/)（loader / driver）+ [`mcp/`](mcp/)（yaml 声明面）

MCP（Model Context Protocol）是 Anthropic 提的一套"LLM ↔ 外部工具"通信协议。**Phase 1（2026-07-17）之后，本项目集成方式从"main.go 里两个硬编码调用"转成"声明式 loader + driver 抽象"**。**Phase 2 起，`mcp.LoadAll` 返回的 `[]io.Closer` 由 `agents.Supervisor` 拥有**（不再是 `main.go` 直接持有），driver + loader 分层保持不变。设计动机与决策见：

- Spec：[`docs/specs/phase-1-mcp-declarative-loading.md`](docs/specs/phase-1-mcp-declarative-loading.md)
- ADR：[`docs/adr/005-mcp-driver-abstraction.md`](docs/adr/005-mcp-driver-abstraction.md)

### 6.1 分层结构

```
cmd/server/main.go
  └── mcp.LoadAll(ctx, MCP_DIR, reg)          ← 一次调用启动所有 MCP
        │
        ▼
  internal/mcp/loader.go
    ├─ 扫 mcp/*.yaml（dictionary order，可重现的启动顺序）
    ├─ 每份 yaml：parseConfig → validate → parseCondition
    ├─ enabled_if.eval() == false → 跳过 + 一行日志
    ├─ enabled_if.eval() == true  → lookupDriver(cfg.Transport).Start(...)
    └─ 任一 Start 失败 → close 已启的 + 返回硬错误（fail-fast，pod crashloop）
        │
        ▼
  internal/mcp/driver.go   (Driver interface)
        ...
        ├──────────────────────────────┐
        ▼                              ▼
  internal/mcp/inproc/            internal/mcp/stdio/
   provider=builtin-demo            通用外部子进程
   (mcp.echo / mcp.list_dir)        (npx / python / go 均可)
```

**关键：cmd/server/main.go 不 import 具体 driver**，而是 blank-import 触发 `init()` 注册：
...
加一个新 transport（sse / http / websocket）= 新增一个 `internal/mcp/<transport>/` 包 + 一行 blank import。**loader 不动。**

### 6.2 Driver interface（`internal/mcp/driver.go`）

```go
type Driver interface {
    Name() string  // 匹配 cfg.Transport ("inproc" | "stdio" | ...)
    Start(ctx context.Context, cfg Config, reg *tools.Registry) (io.Closer, error)
}

func RegisterDriver(d Driver)  // 每个 driver 包 init() 里调用
```

**契约：**

- `Start` 内部完成 MCP 握手 + 拉取 tool list + 用 `cfg.Name + "." + info.Name` 前缀写入 Registry
- 返回 `io.Closer` 让 `main.go` 在优雅关闭时统一回收（stdio 子进程、in-proc pipe 等）
- **返回非 nil error 就是硬错误** —— 不允许 driver 内部把失败降级成 warn（ADR-005 明确关掉了历史上的 warn-and-continue）

### 6.3 Loader 语义（`internal/mcp/loader.go`）

`LoadAll(ctx, dir, reg) ([]io.Closer, error)`：

| 场景 | 结果 |
|---|---|
| `MCP_DIR` 不存在 / 无 yaml | 0 closers, nil error（"没有 MCP" 是合法状态）|
| yaml 解析失败 / transport 未知 / enabled_if 语法错 | 硬错误 |
| `enabled_if` 求值 false | 跳过 + 一行 `mcp: xxx disabled` 日志 |
| `enabled_if` 求值 true 且 `driver.Start` 成功 | 记录 closer + 一行 `mcp: xxx started` 日志 |
| `enabled_if` 求值 true 但 `driver.Start` 失败 | close 已启动的 closers + 返回硬错误 |

**串行启动**：boot 延迟本来就被最慢的 stdio init 支配，串行让失败回收顺序等于启动的反序，实现简单。

### 6.4 Config schema（`internal/mcp/config.go`）

```yaml
name: <string>            # 必填；同时用作 tool 前缀
transport: inproc|stdio   # 必填
enabled_if: <expression>  # 可选，见 §6.5

# --- inproc only ---
provider: <string>        # 必填，选一个内建实现（当前唯一值：builtin-demo）
default_root: <path>      # 可选，透传给 provider（例如 list_dir 兜底目录）

# --- stdio only ---
command: <string>         # 必填
args: [<string>, ...]
env: {KEY: value, ...}
init_timeout: 30s         # 默认 30s，防 npx 首次下载卡死
```

**Fields 不能串**：inproc 设了 `command` 或 stdio 设了 `provider` 都是 parse-time 硬错。

**`${env:VAR}` / `${env:VAR:-default}` 插值**：`command` / `args` / `env` value / `default_root` 都过 `expandVars`。例如 `${env:FS_MCP_ROOT:-.}` 缺环境变量时兜底 `.`。

### 6.5 `enabled_if` 语法（`internal/mcp/cond.go`）

MVP 只支持三种表达式，任何其他写法（包括拼写错误）都是 parse-time 硬错 —— **免得手抖把服务器"静默禁用"**：
...
Phase 2+ 可能扩到 `all: [...]` / `any: [...]` 组合，但保持"启动时求值一次"的语义（不做动态热更新，那是 Phase 2/3 的事）。

### 6.6 tool 命名约定

**tool 名 = `cfg.Name` + `.` + MCP server 返回的 tool 名**。这直接决定 `agents/*.yaml` 里的 `tools:` 引用怎么写。

当前两份声明：

```yaml
# mcp/mcp.yaml —— 内建 demo（in-process）
name: mcp
transport: inproc
provider: builtin-demo
enabled_if: always
default_root: /agents      # list_dir 无参调用时的兜底
# → 注册出 mcp.echo / mcp.list_dir
```

```yaml
# mcp/filesystem.yaml —— 外部 stdio
name: fs
transport: stdio
...
init_timeout: 30s
# → 注册出 fs.read_file / fs.list_directory / ...
```

**关键决策：`filesystem.yaml` 的 `name` 定为 `fs`**（不是 `filesystem`），是为了让既有 `agents/*.yaml` 里对 `fs.read_file` 的引用**零改动**从 Phase 0 迁到 Phase 1。改名会让 registry key 变成 `filesystem.read_file`，`MustResolve` 会跳过并 warn（`fs.` 前缀魔法命名空间见 §5.1），行为回退。

### 6.7 两种 transport 的对比

| 维度 | inproc | stdio |
|---|---|---|
| 部署 | 无额外依赖 | 需要目标语言 runtime（distroless 里没 npx，见 STATUS §2）|
| 性能 | 内存 pipe，几乎零开销 | stdio + JSON-RPC，有序列化开销 |
| 生态 | 你自己写 provider | 现成 MCP servers（filesystem / github / slack …）|
| 崩溃隔离 | 挂了拖 Go 进程一起挂 | 子进程崩了不影响主进程 |
| 演示价值 | 证明 Eino MCP 集成能工作 | 证明能接入生态里现成的服务 |

### 6.8 Phase 1 的边界，与下一步

Phase 1 是**纯启动时决策，运行时 immutable**：yaml 只在 `main.go` 启动时读一次，Registry 里的 tool 集合此后不变。这个限制是有意的 —— 用最少的抽象跑通"声明式"这条脉络，把动态化留给后续 phase。

下一步（见 [`docs/specs/workbuddy-vision.md`](docs/specs/workbuddy-vision.md) §Phase 2/3/4）：

- **Phase 2（2026-07-17，已落地为内部 refactor）**：引入 `agents.Supervisor` + `atomic.Pointer[host.MultiAgent]` + 事务化 `Rebuild`（见 [§7.5](#75-supervisor--phase-2)）。SSE 契约 / evals / agents.yaml / mcp.yaml schema **全不变**，仅在内部把 `main.go` 的 8 步 boot 压成 5 步、把 host 生命周期从裸指针迁到 Supervisor。**Phase 2 不加外部触发接口，Rebuild 只在单元测试里被调用。**
- **Phase 3**：REST API `/api/reload` `/api/agents` `/api/mcp` 挂到 Supervisor 上，把 yaml 声明面搬到运行时 API。
- **Phase 4**：前端 UI 触发这些 REST endpoint，形成完整 workbuddy 控制面。

**Driver interface 已冻结**：新 transport（sse / http / websocket）就是新增一个包 + 一行 blank import，不需要动 loader 或 Config schema。这个"稳定接口 + 可扩展实现"是 ADR-005 想守住的关键收益。

---

## 7. Agent 编排

**文件：** [`internal/agents/build.go`](internal/agents/build.go) + [`internal/agents/supervisor.go`](internal/agents/supervisor.go)（Phase 2 新增）

这是项目最"密"的一节 —— 20 行代码承载了整个多 agent 拓扑，Phase 2 又在外面套了一层 Supervisor 来承担"host 生命周期 + 事务化 rebuild"的职责。

### 7.0 Phase 2 之后的分层全景

```
cmd/server/main.go
  ↓ agents.NewSupervisor(ctx, deps)     ← 5 步启动的核心（原 8 步的 step 3-6 全塞进这里）
  ↓
internal/agents/supervisor.go            ← Phase 2 新增
  ├─ atomic.Pointer[host.MultiAgent]    ← lock-free 读端
  ├─ rebuildMu sync.Mutex               ← 单写者，同时最多一个 Rebuild
  ├─ mcpClosers []io.Closer + closersMu ← grace period 延迟 Close
  ├─ Current()                          ← lock-free read（SSE 首行调）
  ├─ Rebuild(ctx) error                 ← 事务化：scratch registry → build → commit
  └─ Shutdown(ctx) error
  ↓
internal/httpapi/sse.go
  └─ Server{ Sup *agents.Supervisor }   ← 从 HostMA *host.MultiAgent 迁移
      HandleChat 首行：hostMA := s.Sup.Current()
```

**从这一节开始的关键心智模型转变**：`*host.MultiAgent` **不再是** `main.go` / `Server` 结构体里的裸字段，而是"Supervisor 持有的原子指针 + 允许在运行时被替换的资源"。SSE 层拿到的是**一次读的稳定快照**，不是长期引用。
...
Host handoff 事件通过 `host.WithAgentCallbacks` 传入。**Phase 2 起** `sse.go` 里的 host 引用不再从 `Server.HostMA` 字段读，而是请求首行调 `s.Sup.Current()`：

```go
func (s *Server) HandleChat(w http.ResponseWriter, r *http.Request) {
    hostMA := s.Sup.Current()   // ← lock-free 读一次
    // ... 建 sink、装 hostCallback ...
...
**Phase 2 caveat**：`AppendGlobalHandlers` 只在 `main.go` step 4 通过 `httpapi.InstallToolCallbacks()` 调**一次**。**Supervisor.Rebuild 严禁调**这个函数 —— Eino 官方文档明确它不幂等且非线程安全，重复调会导致每次 tool 调用触发 N 份重复事件、并发调可能损坏全局 handler 链。

### 7.5 Supervisor —— Phase 2 的核心新组件

**文件：** [`internal/agents/supervisor.go`](internal/agents/supervisor.go)

Supervisor 是 Phase 2 引入的**编排层新分层**，把原本散落在 `main.go` 里的"MCP loader + agentcfg 加载 + BuildSpecialist × N + BuildHost"合并成一个**事务化的资源装配器**，同时承担 host 的运行时生命周期。设计推理见：

- Spec：[`docs/specs/phase-2-registry-mutation-host-swap.md`](docs/specs/phase-2-registry-mutation-host-swap.md)
- ADR：[`docs/adr/006-registry-mutation-host-swap.md`](docs/adr/006-registry-mutation-host-swap.md)
- Research：[`docs/research/phase-2-registry-mutation-design.md`](docs/research/phase-2-registry-mutation-design.md)、[`docs/research/phase-2-host-swap-risks.md`](docs/research/phase-2-host-swap-risks.md)

#### 数据结构

```go
type Supervisor struct {
    ptr        atomic.Pointer[host.MultiAgent]  // 读端 lock-free
    rebuildMu  sync.Mutex                       // 单写者
    closersMu  sync.Mutex
    mcpClosers []io.Closer                      // 归 Supervisor 拥有
    // ... 依赖：model, registry, agents dir, mcp dir, host prompt ...
}
```

#### 三个核心方法

- **`Current() *host.MultiAgent`**：走 `ptr.Load()`，**lock-free**。SSE 层每个请求首行调一次，之后请求生命周期内都用这一份引用。
- **`Rebuild(ctx) error`**：事务化 rebuild —— 在 scratch registry 上完整 build 出新 host，全部成功后才 `atomic.Store` 提交，同时 swap `mcpClosers`。任何一步失败，旧 host 完全不受影响。**通过 `rebuildMu` 强制单写者**。
- **`Shutdown(ctx) error`**：优雅关闭时 close 所有当前持有的 MCP closers。

#### 事务化 rebuild 的关键不变量

1. **旧 host 引用允许跑完，不 drain**：`atomic.Store` 替换后，in-flight 请求手里的 `*MultiAgent` 引用继续跑；Registry 的 `Unregister` 只删 map entry，tool interface 引用与 Registry 解耦。旧 MCP driver 的 `io.Closer` **延迟 `SUPERVISOR_MCP_GRACE_PERIOD`（默认 30s）后 Close**，给旧请求足够时间完成。
2. **`rebuildMu` 单写者**：同时最多一个 Rebuild 在跑。核心原因是 Eino 的 `callbacks.AppendGlobalHandlers` **不幂等且非线程安全**（官方文档），Rebuild 里绝对不能调；就算不调，多个并发 rebuild 也会让 closers swap 顺序混乱。
3. **`context.WithoutCancel` 防手滑**：Rebuild 内派生 ctx（MCP 子进程 spawn / Ark 客户端 warm-up 等）用 `context.WithoutCancel`，防止上游 caller cancel 导致 rebuild 走一半留下混乱状态。
4. **失败不动旧状态**：scratch registry / 新 closers / 新 host 完全 build 成功才 commit（merge registry + atomic.Store + swap closers），任何一步失败：新建的 closers 就地 Close，`ptr` / `mcpClosers` 保持旧值。

#### Phase 2 承诺的边界

- Phase 2 是**内部 refactor**：SSE 契约 / evals / agents.yaml / mcp.yaml schema 全不变；从外部行为看跟 Phase 1 一样。
- Phase 2 **不加**外部触发接口：Rebuild 只在单元测试里调（`supervisor_test.go`）。
- Phase 3 会加 REST API `/api/reload` `/api/agents` `/api/mcp` 挂到 Supervisor 上，才是"运行时可控制"的开始。
- Phase 4 前端 UI 触发这些 REST endpoint。
...
| **Atomic swap + single writer** | `agents/supervisor.go` | `atomic.Pointer` 无锁读 + `rebuildMu` 单写者，事务化 rebuild（Phase 2）|
...
| **Fail fast** | `main.go:47-77` | 关键组件（LLM / MCP loader）失败即退；MCP 从 Phase 1 起也 fail-fast（[ADR-005](docs/adr/005-mcp-driver-abstraction.md)）|
...
**若走 in-proc：** 在 `internal/mcp/inproc/` 里给 `builtin-demo` provider 加 tool（或加一个新 provider），然后在 `mcp/mcp.yaml` 或新 yaml 里引用 —— 严格来说这个更适合"演示 MCP 协议本身"，不是理想的 MCP 使用方式。**理想是：** 起一个独立的 MCP server 进程（Go / TS / Python 都行），走 stdio 集成 —— 就是 `internal/mcp/stdio/` driver 的目标场景。

**若走外部：** **不需要写 Go 代码**。加一份 `mcp/<name>.yaml`，`transport: stdio`，`command` / `args` 指到 MCP server 二进制，配 `enabled_if` 决定何时启用。loader 会自动加载。**注意**：Phase 1 起 driver.Start 失败即硬错（pod crashloop），不是 warn-and-continue —— 部署环境要么保证依赖齐备，要么把这份 yaml 用 `enabled_if` 关掉。
...
- 后端：`sse.go` 接收 `messages: []Message`（而不是 `q: string`），透传给 `s.Sup.Current().Stream(...)`
...
│   │   └── weather.go
│   ├── mcp/
│   │   ├── config.go       # yaml schema + validate + ${env:VAR} 插值
│   │   ├── cond.go         # enabled_if 表达式（always / env:VAR[=value]）
│   │   ├── driver.go       # Driver interface + RegisterDriver
│   │   ├── loader.go       # LoadAll：扫 mcp/*.yaml → 派发 driver
│   │   ├── inproc/         # transport=inproc driver（builtin-demo provider）
│   │   └── stdio/          # transport=stdio driver（通用外部子进程）
│   ├── agentcfg/loader.go   # YAML 加载
│   ├── agents/
│   │   ├── build.go         # Specialist / Host 构造 + DefaultHostPrompt
│   │   └── supervisor.go    # Phase 2：atomic.Pointer 管 host + 事务化 Rebuild + MCP closers 所有权
│   ├── httpapi/
...
│
├── agents/                  # Specialist YAML 声明面
│   ├── math_agent.yaml
│   ├── ops_agent.yaml
│   └── research_agent.yaml
│
├── mcp/                     # MCP YAML 声明面（Phase 1 新增）
│   ├── mcp.yaml             # inproc builtin-demo
│   └── filesystem.yaml      # stdio npx filesystem
│
├── evals/                   # 路由回归 case
│   ├── routing.yaml
│   └── README.md
...
*本文档最后一次全面对齐代码：2026-07-15（commit `54be7be`）；Phase 2 编排层（Supervisor + atomic.Pointer + 事务化 Rebuild）覆盖至 2026-07-17，见 §3 / §7.5 / §12。若代码有较大重构，请对应更新引用行号。*
